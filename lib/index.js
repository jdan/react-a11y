...
var _runTests = (tests) => {
  return (tagName, props, children, options, onFailure) => {
    tests.map((test) => {
...
var runTests = _runTests([runTagTests, runPropTests]);
var runPostMountTests = _runTests([runLabelTests]);
  ...
  React.createElement = (type, _props, ...children) => {
    ...
    var newProps = assign(
      ...
      isTypeString && {
        ref: function(node) {
          runPostMountTests(
            type, newProps, childrenForTest, options, failureCB);

          // Call the existing ref
          if (props.ref) {
            props.ref(node);
          }
        },
